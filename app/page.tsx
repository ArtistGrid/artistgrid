...
const donationOptions: DonationOption[] = [
  { name: "PayPal", value: "https://paypal.me/eduardprigoana", isUrl: true }, { name: "Patreon", value: "https://www.patreon.com/c/ArtistGrid", isUrl: true }, { name: "Liberapay", value: "https://liberapay.com/ArtistGrid/", isUrl: true }, { name: "Ko-fi", value: "https://ko-fi.com/artistgrid", isUrl: true }, { name: "Bitcoin (BTC)", value: "bc1qn3ufzs4nk62lhfykx78atzjxx8hxptzmrm0ckr", uriScheme: "bitcoin" }, { name: "Ethereum (ETH)", value: "0x0b39d5D190fDB127d13458bd2086cDf950D3034C", uriScheme: "ethereum" }, { name: "Litecoin (LTC)", value: "ltc1q88kpywg3jxxg0jsx9c4e9d8gqs7p07fqptjgtv", uriScheme: "litecoin" }, { name: "Monero (XMR)", value: "bc1qn3ufzs4nk62lhfykx78atzjxx8hxptzmrm0ckr", uriScheme: "monero" },
];
...
const Modal = ({ isOpen, onClose, children, ariaLabel }: { isOpen: boolean; onClose: () => void; children: React.ReactNode; ariaLabel: string; }) => {
  ...
  return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0" onClick={onClose} role="dialog" aria-modal="true" aria-label={ariaLabel} data-state={isOpen ? "open" : "closed"}><div className="bg-neutral-950 border border-neutral-800 shadow-2xl shadow-black/30 rounded-xl w-full max-w-md relative data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-top-[2%] data-[state=open]:slide-in-from-top-[2%]" onClick={(e) => e.stopPropagation()} data-state={isOpen ? "open" : "closed"}><Button variant="ghost" size="icon" onClick={onClose} className="absolute top-3 right-3 text-neutral-500 hover:text-white transition-colors h-8 w-8 rounded-lg" aria-label="Close popup"><X className="w-5 h-5" /></Button>{children}</div></div>);
};

const GallerySkeleton = memo(() => (
  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 sm:gap-6 pt-24">{Array.from({ length: 18 }).map((_, i) => (<div key={i} className="bg-neutral-900 border-neutral-800 rounded-xl p-3"><Skeleton className="aspect-square w-full mb-3 bg-neutral-700 rounded-lg" /><Skeleton className="h-4 w-3/4 bg-neutral-700 rounded-md" /></div>))}</div>
));
...
const ArtistCard = memo(function ArtistCard({ artist, priority, onClick }: { artist: Artist; priority: boolean; onClick: (url: string) => void; }) {
  ...
  return (<div role="link" tabIndex={0} className="bg-neutral-950 border border-neutral-800 hover:border-white/30 hover:bg-neutral-900 hover:-translate-y-1 group rounded-xl overflow-hidden cursor-pointer transition-all duration-300 ease-out hover:shadow-[0_0_30px_rgba(255,255,255,0.12)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-black focus-visible:ring-white focus-visible:shadow-[0_0_30px_rgba(255,255,255,0.12)]" onClick={() => onClick(artist.url)} onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && onClick(artist.url)}><div className="p-0 flex flex-col h-full"><div className="relative aspect-square w-full bg-neutral-900 overflow-hidden"><Image src={`${ASSET_BASE}/${artist.imageFilename}`} alt={artist.name} fill sizes="(max-width: 640px) 50vw, (max-width: 768px) 33vw, (max-width: 1024px) 25vw, (max-width: 1280px) 20vw, 16vw" className="object-cover transition-transform duration-300 ease-out group-hover:scale-105" priority={priority} quality={70} draggable={false} /></div><div className="flex items-start justify-between p-3"><h3 className="font-semibold text-white text-sm leading-tight flex-1 mr-2">{artist.name}</h3>{googleSheetUrl && (<a href={googleSheetUrl} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} aria-label={`Open original Google Sheet for ${artist.name}`} className="flex-shrink-0 p-1 -m-1 rounded-md text-neutral-500 group-hover:text-white transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-white"><FileSpreadsheet className="w-4 h-4" /></a>)}</div></div></div>);
});
...
const Header = memo(({ searchQuery, setSearchQuery, filterOptions, onFilterChange, onInfoClick, onDonateClick }: { searchQuery: string; setSearchQuery: (q: string) => void; filterOptions: FilterOptions; onFilterChange: (k: keyof FilterOptions, v: boolean) => void; onInfoClick: () => void; onDonateClick: () => void; }) => (
...
export default function ArtistGallery() {
  ...
  const [filterOptions, setFilterOptions] = useState<FilterOptions>(() => {
    ...
    try {
      const saved = window.localStorage.getItem('artistGridFilterOptions');
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const response = await fetch("https://artistgrid.cx/backup.csv", { cache: "no-store", signal });
        if (!response.ok) throw new Error("Primary source failed with status: " + response.status);
        ...
        try {
          const response = await fetch("/backup.csv", { cache: "force-cache", signal });
          if (!response.ok) throw new Error("Fallback source also failed");
          const csvText = await response.text();
          setAllArtists(parseCSV(csvText));
          setStatus("success");
        } catch (backupErr) {
          console.error("All data sources failed:", backupErr);
          const message = backupErr instanceof Error ? backupErr.message : "An unknown error occurred.";
    ...
    const loadVisitorCount = async () => {
      try {
        const res = await fetch("https://111224.artistgrid.cx/artistgrid.cx/", { signal });
        if (res.ok) {
          const data = await res.json();
          setVisitorCount(Number(data.count));
        }
      } catch (err) {
        if (err.name !== 'AbortError') console.warn("Visitor count fetch failed:", err);
      }
    };
  ...
  if (status === "loading") {
    return (
      <div className="min-h-screen bg-black text-white">
        <header className="sticky top-0 z-30 py-4 bg-black/70 backdrop-blur-lg border-b border-neutral-900 mb-8">
            <div className="max-w-7xl mx-auto flex items-center gap-4 px-4 sm:px-6">
                <Skeleton className="h-10 w-10 sm:hidden rounded-lg bg-neutral-800" />
                <Skeleton className="h-10 w-10 sm:hidden rounded-lg bg-neutral-800" />
