...
const LyricsPanel = lazy(() => import("@/src/components/lyrics-panel").then((m) => ({ default: m.LyricsPanel })));
...
export const GlobalPlayer = memo(function GlobalPlayer() {
  ...
  const handleQueueRemove = useCallback((index: number) => removeFromQueue(index), [removeFromQueue]);
  const handlePlayFromQueue = useCallback((index: number) => playFromQueue(index), [playFromQueue]);
            ...
            <div className="glass-elevated rounded-2xl overflow-hidden">
              <div className="flex items-center gap-1 px-3 py-2 sm:gap-3 sm:px-4 sm:py-2.5">
                ...
                <div className="hidden sm:flex flex-1 items-center gap-2 min-w-0">
                  ...
                  />
                  <span className="text-xs text-white/30 tabular-nums w-8 flex-shrink-0">{formatTime(duration)}</span>
                </div>
                <VolumeControl
                  volume={state.volume}
                  isMuted={isMuted}
                ...
                <div className="flex items-center gap-0 sm:gap-0.5 flex-shrink-0">
                  <Button
                    ...
                    className={`rounded-xl w-8 h-8 transition-colors ${lyricsOpen ? "text-white hover:bg-white/10" : "text-white/30 hover:text-white hover:bg-white/10"}`}
                    aria-label="Lyrics"
                  >
                  ...
                  <Button
                    ...
                    className={`rounded-xl w-8 h-8 relative transition-colors ${state.queue.length > 0 ? "text-white hover:bg-white/10" : "text-white/30 hover:text-white hover:bg-white/10"}`}
                    aria-label="Queue"
                  >
