...
import { computeETag, fetchWithFallback } from "@/src/lib/api";
...
describe("ETag support", () => {
  ...
  const flushETag = () => new Promise<void>((r) => setTimeout(r, 20));
  ...
  describe("fetchWithFallback", () => {
    ...
    it("handles concurrent requests to different endpoints", async () => {
      fetchSpy.mockResolvedValueOnce(jsonResponse({ a: 1 })).mockResolvedValueOnce(jsonResponse({ b: 2 }));
      const [r1, r2] = await Promise.all([fetchWithFallback("/sh/a/"), fetchWithFallback("/sh/b/")]);
    ...
    it("retries without etag when a 304 arrives for an evicted body", async () => {
      for (let i = 0; i < 60; i++) {
        fetchSpy.mockResolvedValueOnce(jsonResponse({ i }));
        await fetchWithFallback(`/sh/fill-${i}/`);
      }
      await flushETag();
      fetchSpy
      ...
      const [, retryOpts] = fetchSpy.mock.calls[61];
      expect(retryOpts?.headers?.["If-None-Match"]).toBeUndefined();
    });
  });
});
