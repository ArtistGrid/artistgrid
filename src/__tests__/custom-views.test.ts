...
import { getCustomViews, saveCustomViews, addCustomView, deleteCustomView } from "../lib/custom-views";
...
describe("mergeTabData", () => {
  ...
  it("concatenates tracks within the same named era", () => {
    ...
    const merged = mergeTabData([
      { ...base(), eras: { "1": era } },
      { ...base(), eras: { "1": { name: "Dups", data: { Default: [track("y")] } } } },
    ]);
  ...
  it("merges _flat eras", () => {
    ...
    const merged = mergeTabData([
      { ...base(), eras: { _flat: flat } },
      { ...base(), eras: { _flat: { name: "_flat", data: { Default: [track("f2")] } } } },
    ]);
