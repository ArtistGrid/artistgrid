...
describe("DraggablePanel", () => {
  ...
  it("moves the panel with arrow keys", () => {
    const { container } = render(
      <DraggablePanel label="Mini Player" onClose={() => {}}>
        <div>body</div>
      </DraggablePanel>
    ...
    const handle = screen.getByLabelText(/Drag to reposition player/);
    const before = (container.firstChild as HTMLElement).style.left;
    fireEvent.keyDown(handle, { key: "ArrowLeft" });
    const after = (container.firstChild as HTMLElement).style.left;
