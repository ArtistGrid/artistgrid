...
describe("favourites", () => {
  ...
  describe("getFavourites", () => {
    ...
    it("returns empty array for non-array JSON", () => {
      localStorage.setItem("artistgrid-favourites_test-tracker", JSON.stringify("not-an-array"));
      expect(getFavourites(TRACKER_ID)).toEqual([]);
    });
    it("returns saved favourites", () => {
      const favs = ["https://example.com/1", "https://example.com/2"];
      localStorage.setItem("artistgrid-favourites_test-tracker", JSON.stringify(favs));
  ...
  describe("isFavourited", () => {
    ...
    it("returns true when url is favourited", () => {
      localStorage.setItem("artistgrid-favourites_test-tracker", JSON.stringify(["https://example.com/1"]));
  ...
  describe("toggleFavourite", () => {
    ...
    it("preserves order when toggling", () => {
      ...
      toggleFavourite(TRACKER_ID, "https://example.com/2");
      expect(getFavourites(TRACKER_ID)).toEqual(["https://example.com/1", "https://example.com/3"]);
    });
  });
  describe("clearFavourites", () => {
    ...
    it("does not affect other trackers", () => {
      ...
      clearFavourites(TRACKER_ID);
      expect(getFavourites(TRACKER_ID)).toEqual([]);
      expect(getFavourites("other-tracker")).toEqual(["https://example.com/2"]);
    });
  });
  describe("getFavouritedTracks", () => {
    ...
    it("returns tracks in chronological era/track order regardless of favourites order", () => {
      const favs = ["https://example.com/3", "https://example.com/1"];
    ...
    it("skips urls not found in data", () => {
      const favs = ["https://example.com/1", "https://nonexistent.com/99"];
      const result = getFavouritedTracks(TRACKER_DATA, favs);
      expect(result).toHaveLength(1);
      expect(result[0].track).toBe(TRACK_1);
    });
    it("returns tracks in era order, not user-click order", () => {
      const favs = ["https://example.com/2", "https://example.com/1", "https://example.com/3"];
      const result = getFavouritedTracks(TRACKER_DATA, favs);
  ...
  describe("url normalization consistency", () => {
    const LEGACY_TRACK = {
      name: "Legacy",
      url: "https://pillowcase.su/f/abc123",
    } as TALeak;
    const LEGACY_ERA = {
      name: "Legacy Era",
      data: { Default: [LEGACY_TRACK] },
    } as unknown as Era;
    it("toggleEraFavourite stores normalized URLs", () => {
      toggleEraFavourite(TRACKER_ID, LEGACY_ERA);
      expect(getFavourites(TRACKER_ID)).toContain("https://pillows.su/f/abc123");
    });
    it("isEraFavourited matches a favourite saved from the player UI", () => {
      toggleFavourite(TRACKER_ID, "https://pillows.su/f/abc123");
      expect(isEraFavourited(TRACKER_ID, LEGACY_ERA)).toBe(true);
    });
    it("getFavouritedTracks finds tracks whose raw url needs normalization", () => {
      toggleFavourite(TRACKER_ID, "https://pillows.su/f/abc123");
      const result = getFavouritedTracks({ eras: { e: LEGACY_ERA } }, getFavourites(TRACKER_ID));
      expect(result).toHaveLength(1);
      expect(result[0].track.name).toBe("Legacy");
    });
    it("skips tracks with no valid URLs entirely", () => {
      const noUrlTrack = { name: "No URL", quality: "320kbps" } as TALeak;
      const era = { name: "E", data: { Default: [noUrlTrack] } } as unknown as Era;
      toggleEraFavourite(TRACKER_ID, era);
      expect(getFavourites(TRACKER_ID)).toEqual([]);
    });
  });
});
