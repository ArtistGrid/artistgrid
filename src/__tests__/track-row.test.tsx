...
describe("TrackRow", () => {
  ...
  it(
    "opens the actions menu and triggers share / play next / add to queue / download",
    { timeout: 15000 },
    async () => {
      ...
      const { rerender } = render(
        <TrackRow {...makeProps({ handleShareTrack, handlePlayNext, handleAddToQueue, handleDownload })} />
      );
      const reopen = () => userEvent.click(screen.getByLabelText("Track actions"));
      await reopen();
      ...
      rerender(<TrackRow {...makeProps({ handleShareTrack, handlePlayNext, handleAddToQueue, handleDownload })} />);
  ...
  it("opens original and toggles favourite from the menu", async () => {
    ...
    render(<TrackRow {...makeProps({ handleOpenOriginal, handleToggleFavourite })} />);
