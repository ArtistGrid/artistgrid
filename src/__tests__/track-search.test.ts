...
describe("track-search", () => {
  ...
  it("combines fuzzy and exact substring matches", () => {
    ...
    // Exact match still works seamlessly
    const exact = searchTracks(items, "Alien", fuseInstance);
    expect(exact.has(t3)).toBe(true);

    // 1-character query falls back cleanly to substring even with Fuse instance
    const oneChar = searchTracks(items, "h", fuseInstance);
    expect(oneChar.has(t2)).toBe(true); // flas**h**ing
    expect(oneChar.has(t4)).toBe(true); // **h**eartbreaker
  });
});
