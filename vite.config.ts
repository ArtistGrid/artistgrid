...
async function minifyJs(dir: string) {
	...
	for (const file of files) {
		...
		const swcOut = await swc.minify(code, {
			compress: { passes: 5, toplevel: true, unsafe: true, unsafe_math: true, unsafe_methods: true, unsafe_proto: true, drop_console: false, pure_getters: true, hoist_funs: true, hoist_vars: true, reduce_vars: true, collapse_vars: true, join_vars: true, booleans_as_integers: true },
		...
		try {
			const terserOut = await terserMod.default.minify(code, {
				...
				module: true,
				toplevel: true,
				compress: { passes: 3, toplevel: true, unsafe: true, unsafe_math: true, arguments: true, hoist_funs: true, hoist_vars: true, reduce_vars: true, collapse_vars: true, join_vars: true, booleans_as_integers: true, drop_console: false, pure_getters: true },
				mangle: { toplevel: true },
		...
		try {
			const uglifyOut = uglify.minify(code, {
				compress: { passes: 5, toplevel: true, unsafe: true, arguments: true, hoist_funs: true, hoist_vars: true, reduce_vars: true, collapse_vars: true, join_vars: true, booleans_as_integers: true, pure_getters: true, unsafe_math: true },
				mangle: { toplevel: true },
				output: { comments: false, wrap_func_args: false },
				module: true,
			});
...
async function minifyCss(dir: string) {
	...
	for (const file of files) {
		...
		try {
			...
			]).process(code, { from: undefined }).then((r) => r.css);
		} catch {}
...
function customMinifyPlugin(dir: string) {
	return {
		...
		async writeBundle() {
			...
			console.log(
				`  custom-minify: JS -${(jsSaved / 1024).toFixed(1)} KB, CSS -${(cssSaved / 1024).toFixed(1)} KB`,
			);
		},
	};
}
	...
	plugins: [
		...
		VitePWA({
			...
			manifest: {
				...
				description:
					"Discover and track unreleased music from your favorite artists.",
				theme_color: "#000000",
